...
module.exports = ({ bot }) => {
    ...
    bot.on("guildMemberAdd", async (guild, member) => {
      ...
      if (thread != null) {
        await thread.postSystemMessage(
          `***Der Benutzer hat ${guild.name} betreten.***`
        );
      }
    });
  }
      ...
      setTimeout(async () => {
        ...
        if (thread != null) {
          if (leaveIgnoreIDs.includes(member.id)) {
            ...
            await thread.postSystemMessage(
              `***Der Benutzer hat ${guild.name} verlassen.***`
            );
          }
        }
      }, 2 * 1000);
  ...
  if (config.notifyOnMainServerLeave) {
    bot.on("guildBanAdd", async (guild, user) => {
      ...
      if (thread != null) {
        await thread.postSystemMessage(
          `***Der Benutzer wurde von ${guild.name} gebannt.***`
        );
  ...
  if (config.notifyOnMainServerJoin) {
    bot.on("guildBanRemove", async (guild, user) => {
      ...
      if (thread != null) {
        await thread.postSystemMessage(
          `***Der Benutzer wurde von ${guild.name} entbannt.***`
        );
      }
    });
  }
