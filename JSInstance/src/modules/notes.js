...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  async function userNotesCmd(msg, userId) {
    ...
    for (const userNote of userNotes) {
      ...
      const content = [
        `Gesetzt von <@!${userNote.author_id}> um <t:${timestamp}:f>:`,
        `${START_CODEBLOCK}${escapeMarkdown(userNote.body)}${END_CODEBLOCK}`,
        `*Lösche die Notiz mit \`${config.prefix}delete_note ${userNote.id}\`*\n`,
      ].join("\n");
  ...
  async function addUserNoteCmd(msg, userId, body) {
    ...
    await msg.channel.createMessage({
      content: `Notiz hinzugefügt für <@!${userId}>`,
      allowedMentions: {},
    });
  }
  ...
  async function deleteUserNoteCmd(msg, noteId) {
    ...
    if (! note) {
      postError(msg.channel, "Notiz nicht gefunden!");
      return;
    }

    await deleteNote(noteId);
    await msg.channel.createMessage(`Notiz gelöscht über <@!${note.user_id}>:\n${START_CODEBLOCK}${escapeMarkdown(note.body)}${END_CODEBLOCK}`);
  }
