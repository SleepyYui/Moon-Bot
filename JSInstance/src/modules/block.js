...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  const blockCmd = async (msg, args, thread) => {
    ...
    if (isBlocked) {
      channel.createMessage("Benutzer ist bereits gesperrt.");
      return;
    }
    ...
    if (expiresAt) {
      ...
      msg.channel.createMessage(`<@${userIdToBlock}> (id \`${userIdToBlock}\`) wurde für undefinierte Zeit gesperrt.`);
  ...
  const unblockCmd = async (msg, args, thread) => {
    ...
    if (unblockAt) {
      ...
      msg.channel.createMessage(`<@${userIdToUnblock}> (id \`${userIdToUnblock}\`) wird entblockt in ${humanized}`);
      ...
      msg.channel.createMessage(`<@${userIdToUnblock}> (id ${userIdToUnblock}) wurde entblockt`);
    ...
    if (blockStatus.isBlocked) {
      if (blockStatus.expiresAt) {
        msg.channel.createMessage({
          content: `<@!${userIdToCheck}> (id \`${userIdToCheck}\`) ist geblockt bis ${blockStatus.expiresAt} (UTC)`,
          allowedMentions: { users: [userIdToCheck] },
        });
      } else {
        msg.channel.createMessage({
          content: `<@!${userIdToCheck}> (id \`${userIdToCheck}\`) ist für undefinierte Zeit blockiert.`,
          allowedMentions: { users: [userIdToCheck] },
        });
      }
    } else {
      msg.channel.createMessage({
        content: `<@!${userIdToCheck}> (id \`${userIdToCheck}\`) ist nicht blockiert`,
        allowedMentions: { users: [userIdToCheck] },
      });
    }
  });
