...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxThreadCommand("move", "<category:string$>", async (msg, args, thread) => {
    ...
    if (containsRankings[0][1] === 0) {
      thread.postSystemMessage("Keine passende Kategorie");
      return;
    }
    ...
    try {
      ...
      thread.postSystemMessage(`Kann nicht verschieben: ${e.message}`);
      return;
    }
    ...
    if (config.syncPermissionsOnMove) {
      ...
      try {
        ...
        thread.postSystemMessage(`Thread verschoben zu ${targetCategory.name.toUpperCase()}, jedoch ohne folgende Berechtigungen: ${e.message}`);
        return;
      }
    }

    thread.postSystemMessage(`Verschoben zu ${targetCategory.name.toUpperCase()}`);
  });
};
