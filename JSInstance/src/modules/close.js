...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  async function applyScheduledCloses() {
    ...
    for (const thread of threadsToBeClosed) {
      ...
      await sendCloseNotification(thread, `Modmail thread #${thread.thread_number} mit ${thread.user_name} (${thread.user_id}) wurde wie geplant von ${thread.scheduled_close_name} geschlossen.`);
    }
  }
  ...
  commands.addGlobalCommand("close", "[opts...]", async (msg, args) => {
    ...
    if (msg.channel instanceof Eris.PrivateChannel) {
      ...
      await messageQueue.add(async () => {
        thread.postSystemMessage("Thread vom User geschlossen. Schließt Thread...");
      ...
      if (args.cancel || opts.includes("cancel") || opts.includes("c")) {
        ...
        if (thread.scheduled_close_at) {
          ...
          thread.postSystemMessage("Geplantes Schließen abgebrochen");
        }
      ...
      if (delayStringArg) {
        ...
        if (delay === 0 || delay === null) {
          thread.postSystemMessage("Ungültige Zeitspanne. z.B.: \"1h30m\"");
          return;
        }
    ...
    await sendCloseNotification(thread, `Modmail thread #${thread.thread_number} mit ${thread.user_name} (${thread.user_id}) wurde geschlossen von ${closedBy}`);
  }, {
    options: [
  ...
  bot.on("channelDelete", async (channel) => {
    ...
    console.log(`[INFO] Thread mit ${thread.user_name} wird automatisch geschlossen da der Kanal gelöscht wurde.`);
    ...
    await sendCloseNotification(thread, `Modmail thread #${thread.thread_number} mit ${thread.user_name} (${thread.user_id}) wurde automatisch geschlossen da der Kanal gelöscht wurde.`);
  });
};
